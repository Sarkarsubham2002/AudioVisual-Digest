...
#for audio file directory########################
audio_path =r"../musicdata/hello1.mp3"
...
def ml_func():
    ...
    parser.add_argument('--model', type=str, default='small', choices="tiny.en,tiny,base.en,base,small.en,small,medium.en,medium,large-v1,large-v2,large".split(","),help="Name size of the Whisper model to use (default: large-v2). The model is automatically downloaded from the model hub if not present in model cache dir.")
    ...
    def output_transcript(o, now=None):
        if now is None:
            now = time.time()-start
        if o[0] is not None:
            print("%s" % (o[2]),file=logfile,flush=True)
            print("%s" % (o[2]),flush=True)
        else:
    ...
    result_string = summarized_text

    tts = gTTS(result_string, lang='en')
    # Save converted audio as mp3 format
    tts.save('../sumaudio/audio.mp3')
